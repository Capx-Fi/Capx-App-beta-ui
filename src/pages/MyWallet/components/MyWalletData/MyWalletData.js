    ...
    <div className="wallettable w-full fs-14 text-cgreen-700 ">
      <table className="wallet-theading flex flex-col gap-2 font-black hidden md:flex">
        <tr className="wallet-th2 tr flex flex-row border-2 py-4 px-4 rounded-2xl">
          ...
          <td className="th quest-details text-start opacity-80">Quest Details</td>
      ...
      <tr className="wallet-tbody flex flex-col rounded-2xl border-2 font-semibold pb-4 pt-2 opacity-90 ">
        {walletData.map((data, ind) => {
          return (
            <td className={`wallet-tr flex px-4 py-3 flex-row border-b-2 row${ind + 1}`}>
              <td className="td serial">{data.serial}</td>
              <td className="td quest-details text-start">{data.taskDetails}</td>
              <td className="td quest-category text-start">{data.taskCategory}</td>
              <td className="td quest-date text-start">{data.completionDate}</td>
      ...
      {walletData.map((data, ind) => {
          return (
          <div className={`wallet-tr fs-14 flex px-4 py-3 font-bold flex-col border-b-2 gap-2 md:hidden opacity-80 row${ind + 1}`}>
          <div className="serialno"><span className="inline opacity-80 font-semibold">Task No : </span>  {data.serial}</div>
          <div className="questDetails"><span className="inline opacity-80">Task Details : </span> {data.taskDetails}</div>
          <div className="questCategory"><span className="inline opacity-80">Task Category : </span> {data.taskCategory}</div>
          <div className="questDate"><span className="inline opacity-80">Completed on : </span> {data.completionDate}</div>
          <div className="questRewards"><span className="inline opacity-80">Total Earned : </span> {data.earned}</div>
