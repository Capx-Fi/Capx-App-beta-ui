...
const Codestep1 = ({ actionData }) => {
  ...
  const ogInviteCode = useSelector((state) => state.user.og_invite_code);
  ...
  const getQuestInviteCode = () => {
    if (actionData.action_order_type === "Generate_Invite_Code") {
  ...
  const handleshareModal = () => {
    ...
    navigator.clipboard.writeText(
      `Join the Capx app closed alpha - app.capx-fi with my code - ${getQuestInviteCode()}`
    );
  ...
  const handleCopyTextButton = () => {
    navigator.clipboard.writeText(getQuestInviteCode());
    setShowCopiedBox(true);
    setTimeout(() => {
      setShowCopiedBox(false);
    }, 1500);
