import React from "react";
import AliceCarousel from 'react-alice-carousel';
import 'react-alice-carousel/lib/alice-carousel.css';
import {goldcoin,peopleicon} from "../../../../assets/images/home";

const handleDragStart = (e) => e.preventDefault();

const OldTasks = () => {
  const dailytaskdata = [
    {
      tasktitle: "How to earn xCapx?",
      taskreward: "1 xCapx",
      taskcategory: "oldtasks",
      taskpeople: "72",
    },
    {
      tasktitle: "Generate invite code",
      taskreward: "1 xCapx",
      taskcategory: "oldtasks",
      taskpeople: "32",
    },
    {
      tasktitle: "Tweet from your account",
      taskreward: "1 xCapx",
      taskcategory: "oldtasks",
      taskpeople: "13",
    },
    {
      tasktitle: "How to earn xCapx?",
      taskreward: "1 xCapx",
      taskcategory: "oldtasks",
      taskpeople: "72",
    },
    {
      tasktitle: "Generate invite code",
      taskreward: "1 xCapx",
      taskcategory: "oldtasks",
      taskpeople: "32",
    },
    {
      tasktitle: "Tweet from your account",
      taskreward: "1 xCapx",
      taskcategory: "oldtasks",
      taskpeople: "13",
    },
    
  ];

  return (
    <div className="oldtasks">
      <div className="oldtasks-wrapper flex flex-col gap-12 pr-8 md:flex-col">
        {dailytaskdata.map((data, ind) => {
          return (
            <div className={`oldtasks-card text-cgreen-600 border-x-2 border-t-2 border-b-8 rounded-2xl fs-14 font-semibold w-full p-5 flex flex-col gap-4 row${ind + 1}`}>
              <div className="oldtask-row-1 flex flex-col gap-3 md:flex-row w-full justify-between">
                  <div className="oldtask-chip flex flex-row py-1 px-2 gap-1 items-center rounded-xl border-2">
                    <img src={goldcoin} className="w-5"></img>
                    <p className="fs-13">{data.taskreward}</p>
                  </div>

                  <div className="oldtask-chip flex flex-row p-2 gap-1 items-center rounded-xl border-2">
                    <p className="fs-13">Completed by {data.taskreward}</p>
                    <img src={peopleicon} className="w-5"></img>
                  </div>
              </div>
              <p className="oldtask-title fs-21 px-2 pt-4">{data.tasktitle}</p>
            </div>
          );
        })}
      </div>
    </div>
  );
};

export default OldTasks;
